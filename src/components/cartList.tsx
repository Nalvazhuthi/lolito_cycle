...
const CartList = () => {
    ...
    return (
        <div
            ref={cartRef}
            className="cart-list fixed top-0 right-0 h-full w-[360px] bg-white bg-opacity-90 shadow-2xl z-50 border-l border-gray-300"
                ...
                {cartItems.length === 0 ? (
                    ...
                    <>
                        {cartItems.map((item) => (
                            <div
                                key={item.id}
                                className="flex items-start gap-5 border-b border-gray-100 pb-6 last:border-b-0"
                            >
                                {/* Image */}
